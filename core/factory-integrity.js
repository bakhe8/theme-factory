...
function scanPolicyCompliance() {
  ...
  for (const exception of result.exceptions || []) {
    exceptions.push({ ...exception, type: `Policy:${exception.type}` });
  }
}
...
- **الاستثناءات المقبولة:** ${exceptions.length}
...
${exceptions.length === 0 ? '*لا توجد استثناءات.*' : exceptions.map(formatFinding).join('\n')}

---

## 📜 التوصية النهائية
${issues.length === 0
  ? 'الثيم جاهز للمرحلة التالية من البناء والمعاينة. يظل التحقق النهائي عبر مراجعة سلة اليدوية مطلوباً.'
