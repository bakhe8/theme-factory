...
أي ثيم جديد يبدأ من `themes/raed` فقط، لكن ليس بنسخ يدوي. المصنع ينسخ رائد عبر `create/manufacture`، يكتب توقيع `.factory/manifest.json`، ثم يطبق المواصفات والقدرات المسجلة. أي `generated-theme` لا يحمل `source_template: raed` يفشل في `workflow gate`.
...
قبل إنشاء مجلد تسليم نهائي يجب أن يوجد قرار تسليم يدوي موثق أو نتيجة مراجعة سلة سابقة:

```bash
node factory.js salla-review gate <theme-name>
```

هذه البوابة مرتبطة بـ `deliver` لا بـ `certify`: الاعتماد المحلي يثبت جاهزية المصنع، أما التسليم اليدوي لسلة فيحتاج قرار إرسال صريحا يطابق بصمة الثيم. نتيجة سلة الحقيقية تسجل لاحقا عندما ترجع من المراجعة.
