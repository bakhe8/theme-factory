...
class Cart extends BasePage {
    ...
    updateCartPageInfo(cartData) {
        ...
        app.subTotal.textContent = salla.money(cartData.sub_total);
        if(app.taxAmount) 
          app.taxAmount.textContent = salla.money(cartData.tax_amount);
        if (app.orderOptionsTotal) app.orderOptionsTotal.textContent = salla.money(cartData.options_total);
        ...
        app.totalDiscount.querySelector('b').textContent = '- ' + salla.money(cartData.total_discount);
        app.shippingCost.querySelector('b').textContent = salla.money(cartData.real_shipping_cost);
        ...
        app.freeShippingMsg.textContent = isFree
    ...
    updateItemInfo(item) {
        ...
        if (total !== totalElement.textContent) {
        ...
        priceElement.textContent = salla.money(item.price);

        // Update original price when item is on sale
        if (hasSalePrice) {
            itemOriginalPrice.textContent = salla.money(item.original_price);
        }

        if (!hasSpecialPrice){return;}
        if (!newOffersActive) {offerElement.textContent = item.offer.names;}
        regularPriceElement.textContent = salla.money(item.product_price);
    }
}
