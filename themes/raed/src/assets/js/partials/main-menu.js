class NavigationMenu extends HTMLElement {
    ...
    */
    getMobileMenu(menu, displayAllText) {
        ...
        const url = this.escapeHTML(menu.url || '#');
        const image = this.escapeHTML(menu.image || '');
        const displayAll = this.escapeHTML(displayAllText || '');
        const menuImage = image ? `<img src="${image}" class="rounded-full" width="48" height="48" alt="${title}" />` : '';

        return `
        <li class="lg:hidden text-sm font-bold" ${menu.attrs}>
            ${!this.hasChildren(menu) ? `
                <a href="${url}" aria-label="${title || 'category'}" class="text-gray-500 ${menu.image ? '!py-3' : ''}" ${menu.link_attrs}>
                    ${menuImage}
                    <span>${title}</span>
                </a>` :
                `
                ...
                <ul>
                    <li class="text-sm font-bold">
                        <a href="${url}" class="text-gray-500">${displayAll}</a>
                    </li>
    ...
    */
    getDesktopMenu(menu, isRootMenu, additionalClasses = '') {
        ...
        <li class="${this.getDesktopClasses(menu, isRootMenu)} ${additionalClasses}" ${menu.attrs} data-menu-item>
            <a href="${url}" aria-label="${title || 'category'}" ${menu.link_attrs}>
                <span>${title}</span>
            </a>
            ${this.hasChildren(menu) ? `
    ...
    */
    createMoreDropdown() {
        ...
        <li class="!hidden lg:!block root-level lg:!inline-block has-children relative" id="more-menu-dropdown">
            <a href="#" aria-label="${this.escapeHTML(this.moreText)}">
                <span>${this.escapeHTML(this.moreText)}</span>
            </a>
