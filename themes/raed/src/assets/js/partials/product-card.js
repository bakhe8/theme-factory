...
class ProductCard extends HTMLElement {
  ...
  render(){
    ...
    this.isInWishlist = !salla.config.isGuest() && salla.storage.get('salla::wishlist', []).includes(Number(this.product.id));
      ...
      const productSubtitle = this.escapeHTML(this.product?.subtitle || '');
      const imageUrl = this.escapeHTML(this.product?.image?.url || this.product?.thumbnail || this.placeholder || '');
      const imageAlt = this.escapeHTML(this.product?.image?.alt || this.product.name);
      const donationAmount = this.escapeHTML(this.donationAmount || '');
      const addButtonLabel = this.escapeHTML(this.product.add_to_cart_label ? this.product.add_to_cart_label : this.getAddButtonLabel());

      this.innerHTML =  `
        <div class="${!this.fullImage ? 's-product-card-image' : 's-product-card-image-full'}">
          <a href="${productUrl}" aria-label="${imageAlt}">
           <img 
        ...
        <div class="s-product-card-content">
          ...
          <div class="s-product-card-content-main ${this.isSpecial ? 's-product-card-content-extra-padding' : ''}">
            ...
            ${this.product?.subtitle && !this.minimal ?
              `<p class="s-product-card-content-subtitle opacity-80">${productSubtitle}</p>`
              : ``}
          </div>
          ...
          <div class="s-product-card-donation-input">
            ${this.product?.donation?.can_donate && this.product?.donation?.custom_amount_enabled  ?
              `<label for="donation-amount-${this.product.id}">${donationAmount} <span>*</span></label>
              <input
                type="text"
                onInput="${e => {
