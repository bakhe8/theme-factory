...
class WishlistCard extends HTMLElement {
    ...
    render() {
        ...
        const imageUrl = this.escapeHTML(this.product.image?.url || '');
        const imageAlt = this.escapeHTML(this.product.image?.alt || this.product.name || '');

        this.innerHTML =  `
        <div class="flex items-center mb-4 sm:mb-0">
          <a href="${productUrl}" class="product-entry__image">
            <img class="object-cover w-full h-full" src="${imageUrl}" loading="lazy" alt="${imageAlt}" />
          </a>
